            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <!-- /.card-header -->
                        <form action="{{ route('roleUpdate', $role->id_role) }}" method="POST">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-sm-6">
                                            ...
                                            <select name="id_company" class="form-control select2bs4"
                                                ...
                                                @foreach ($companys as $item)
                                                    <option value="{{ $item->id_company }}"
                                                        {{ $item->id_company === $role->id_company ? 'selected' : '' }}>
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" value="{{ $role->nama_role }}"
                                ...
                                <div class="row">
                                    <div class="col-sm-10">
                                        <div>Created by : {{ $datauser['create'] }} | {{ $role->created_at }}</div>
                                        @if ($datauser['update'] != 0)
                                            <div>Updated by : {{ $datauser['update'] }} | {{ $role->updated_at }}</div>
                                        @endif
                                    </div>
                                    <div class="col-sm-2">
                                        <button type="submit" class="btn btn-primary float-sm-right"><i
                                            class="fas fa-save"></i> Ubah</button></a>
                                    </div>
                                </div>
                            </div>
                        </form>
